...
export default function Register() {
  ...
  async function handleInitialSubmit(e) {
    ...
    if (!validateEmail(normalizedEmail)) {
      setError(safeText("auth.error_invalid_email", "Invalid email address"));
      return;
    }
    ...
    if (formData.role === "seller" && !GST_REGEX.test(formData.gst_number)) {
      setError(safeText("auth.error_gst_format", "Invalid GST number format"));
      return;
    }
      ...
      <div className="register-layout">
        <div className="register-visual">
          ...
          <div className="register-visual-content">
            <span className="register-badge">
              {safeText("auth.join", "Join")}
            </span>

            <h1>{safeText("auth.visual_title", "Welcome to AgroMitra")}</h1>
            ...
            <div className="register-highlights">
              <div className="register-highlight-card">
                <h3>{safeText("auth.buyer_access", "Buyer Access")}</h3>
                <p>
                  {safeText(
                    "auth.buyer_access_p",
              ...
              <div className="register-highlight-card">
                <h3>{safeText("auth.seller_access", "Seller Access")}</h3>
                <p>
                  {safeText(
                    "auth.seller_access_p",
                    "List products, reach buyers faster, and grow online."
                  )}
                </p>
              </div>
            </div>
        ...
        <div className="register-form-side">
          <div className="register-card">
            ...
            <div className="register-bottom">
              <p>
                {safeText("auth.haveAccount", "Already have an account?")}{" "}
                <Link to="/buyer-login">
                  {safeText("auth.buyerLogin", "Buyer Login")}
                </Link>{" "}
                /{" "}
                <Link to="/seller-login">
                  {safeText("auth.sellerLogin", "Seller Login")}
                </Link>
              </p>
            </div>
          </div>
